    ...
    $(document).on(BJUI.eventType.initUI, function(e) {
        ...
        if (KindEditor) {
            $box.find('[data-toggle="kindeditor"]').each(function() {
                ...
                if (options.uploadjson && typeof options.uploadjson == 'string')
                	options.uploadJson = options.uploadjson;
                if (options.filldescafteruploadimage)
                	options.fillDescAfterUploadImage = options.filldescafteruploadimage;
                if (options.pastetype)
                	options.pasteType = options.pastetype;
                if (options.afterupload)
                	options.afterUpload = options.afterupload.toFunc();
                if(options.filemanagerjson)
                	options.fileManagerJson = options.filemanagerjson;
